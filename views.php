...
if(isset($_GET['username']) && isset($_GET['case']))
{
	if($_GET['case'] == "views")
	{
		...
		<div class="container">
            <h1 class="header-tiktok"><i class="fa-brands fa-tiktok"></i> TikTok Views Automation</h1>
...
if(isset($_POST['video_link']) && isset($_POST['round_number']))
{
	...
	else
	{
		...
		{	
			if($_POST['round_number'] > 100) // if it is more than 100
			{
				 ?>
	...
	// 3- get view button location 
	$views_button = $driver->wait(60,500)->until(WebDriverExpectedCondition::presenceOfElementLocated(WebDriverBy::className('t-views-button')));

	// 4- click on view button location
	$views_button->click(); 

	// 5- get search view input locatio by using 'ruto xpath find chrome extension'
	$get_search_input = $driver->findElement(WebDriverBy::cssSelector('html>body>div:nth-of-type(10)>div>form>div>input')); 

	// 6- insert letter by letter in view search input video link
	...
	$driver->findElement(WebDriverBy::cssSelector('html>body>div:nth-of-type(10)>div>form>div>div>button'))->click();
	...
	while ($predefined_round < $user_round)
	{
		...
		if(count($driver->findElements(WebDriverBy::cssSelector('html>body>div:nth-of-type(10)>div>form>div>div>button'))) > 0 ) // means it is existed
		{
		    sleep(5); // sleep 5 seconds to ensure everything is right

		    // check if the search button is enabled 
		    if($driver->findElement(WebDriverBy::cssSelector('html>body>div:nth-of-type(10)>div>form>div>div>button'))->isEnabled())
		    {
		        ...
		        if(count($driver->findElements(WebDriverBy::className('views-countdown'))) > 0) // the timer is here
		        {

		            // search about word ready and we will try searching period about 3 minutes
					$driver->wait(180, 500)->until(WebDriverExpectedCondition::elementTextContains(WebDriverBy::className('views-countdown'), 'READY'));
    
		            // put the value inside a variable
		            $get_ready_value = $driver->findElement(WebDriverBy::className('views-countdown'))->getText();
		            ...
		            if(str_contains($pre_defined_value, $get_ready_value))
		            {
		               ...
		               sleep(5);
		                ...
		                if($driver->findElement(WebDriverBy::cssSelector('html>body>div:nth-of-type(10)>div>form>div>div>button'))->isEnabled())
		                {
		                    ...
		                    // check if the views increase button is existed or not
		                    if(count($driver->findElements(WebDriverBy::cssSelector('div#c2VuZC9mb2xeb3dlcnNfdGlrdG9V>div>div>form>button'))) > 0)
		                    {
		                        // click on views increase button
		                        $driver->findElement(WebDriverBy::cssSelector('div#c2VuZC9mb2xeb3dlcnNfdGlrdG9V>div>div>form>button'))->click();
		                        $round_number_display = $predefined_round+1;
		                        ?>

		                            <div class="alert alert-success text-center mx-auto fit-content">
		                                Successfully 1000 views sent => Round (<?php echo $round_number_display; ?>).
		                            </div>
		        ...
		        } // we find the view increase button directly
		        elseif (count($driver->findElements(WebDriverBy::cssSelector('div#c2VuZC9mb2xeb3dlcnNfdGlrdG9V>div>div>form>button'))) > 0)
		        {
		        	// sleep for 5 seconds
		            sleep(5);
